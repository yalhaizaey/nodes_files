...
function initiate_metric_array {
	...
	metricsValues=('NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA' 'NA')
}
...
function which_program_edge_split_pipeline {
	...
	then
		...
		executeApplication="pocketsphinx_continuous -infile /mnt/assets/$imageName.wav -logfn /dev/null"
...
function calc_rtf {
	length=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 /mnt/assets/$imageName.wav)
...
while getopts :ljyp:bcez: opt; do
    case "$opt" in
        y)
            application=0
            imageName=$OPTARG
